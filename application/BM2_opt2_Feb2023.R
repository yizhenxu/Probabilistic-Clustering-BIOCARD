...
data {
  ...
  int subjvec[Nobs]; // the lth entry is the subject idx to which time[l] belongs to
  ...
  int y1on0[Nobs]; // indicator of MMScore on the lower boundary
  
  real<lower=0> a; 
  real<lower=0> b;
  
  int catlen[3]; //3,4,3
  int catsum[3]; //0,3,7 = cumsum(c(0,catlen))[1:3]
}
parameters {
  ...
  
{
    ...
    for(i in 1:Nsubs){
      ...
      cov1 = cov_exp_quad(timei, 1.0, rho_theta1)+ diag_matrix(rep_vector(1e-10, lsub[i]));
      L_cov1 = cholesky_decompose(cov1);
      cov2 = cov_exp_quad(timei, 1.0, rho_theta2)+ diag_matrix(rep_vector(1e-10, lsub[i]));
...
library(invgamma)
lu = c(min(agemin,na.rm=T), max(agedrt)) #here 0.05393566 2.40711841 -> original0.5393566 24.0711841
foo = function(par){
...
fit = stan(file = "mod0.stan", data = staninput,
...
save(fit, file = paste0("BM2_opt2_Feb2023_noWMH.RData"))  #here
#
